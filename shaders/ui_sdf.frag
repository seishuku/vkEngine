...
void main()
{
	...
	switch(Type)
	{
		case UI_CONTROL_BUTTON:
		{
			...
			float shadow=sdfDistance(abs(sdRoundedRect(UV+vec2(-0.01, 0.01), aspect-0.02, corner_radius-0.01))-0.02);
		...
		{
			float dist_ring=abs(sdRoundedRect(UV-vec2(0.01), aspect-0.02, corner_radius))-0.02;
			float ring=sdfDistance(dist_ring);

			float dist_shadow=abs(sdRoundedRect(UV+vec2(0.01), aspect-0.02, corner_radius))-0.02;
			float shadow=sdfDistance(dist_shadow);
			...
			if(Color.w>(UV.x/aspect.x)*0.5+0.5)
				center_alpha=sdfDistance(sdRoundedRect(UV, aspect-0.04, corner_radius-0.02));;
